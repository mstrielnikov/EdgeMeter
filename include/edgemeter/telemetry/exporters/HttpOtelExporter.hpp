#ifndef HTTP_OTEL_EXPORTER_HPP
#define HTTP_OTEL_EXPORTER_HPP
...
// ---------------------------------------------------------------------------
template <>
class HttpOtelExporter<Plain> {
public:
    explicit HttpOtelExporter(core::Config config) : config_(std::move(config)) {}
    ~HttpOtelExporter() = default;
...
static_assert(telemetry::Exporter<HttpOtelExporter<Plain>>,  "HttpOtelExporter<Plain> must satisfy telemetry::Exporter");
static_assert(telemetry::Exporter<HttpOtelExporter<Secure>>, "HttpOtelExporter<Secure> must satisfy telemetry::Exporter");

} // namespace telemetry

#endif // HTTP_OTEL_EXPORTER_HPP
